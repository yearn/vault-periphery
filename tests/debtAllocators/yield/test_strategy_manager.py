...
def test_strategy_manager_setup(strategy_manager, mock_tokenized, daddy, yield_manager):
    assert strategy_manager.strategyOwner(mock_tokenized) == ZERO_ADDRESS
    assert strategy_manager.yieldManager() == yield_manager
    assert strategy_manager.governance() == daddy
...
def test_add_new_strategy(
    ...
    with ape.reverts("!governance"):
        strategy_manager.manageNewStrategy(mock_tokenized, sender=management)

    with ape.reverts("!pending"):
        strategy_manager.manageNewStrategy(mock_tokenized, sender=daddy)

    mock_tokenized.setPendingManagement(strategy_manager, sender=management)

    tx = strategy_manager.manageNewStrategy(mock_tokenized, sender=daddy)
    ...
    assert strategy_manager.strategyOwner(mock_tokenized) == management
    assert strategy_manager.yieldManager() == yield_manager
...
def test_update_owner(
    ...
    assert strategy_manager.strategyOwner(mock_tokenized) == user
    assert strategy_manager.yieldManager() == yield_manager
    assert mock_tokenized.management() == strategy_manager
...
def test_record_full_profit(
    ...
    strategy_manager.manageNewStrategy(mock_tokenized, management, sender=daddy)
    ...
    # Profit should be fully locked
    assert mock_tokenized.totalAssets() == to_deposit + profit
    assert mock_tokenized.totalSupply() == to_deposit + profit
