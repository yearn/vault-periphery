...
def test_setup(daddy, vault, strategy, refund_accountant, fee_recipient):
    ...
    assert accountant.rewardRefund(vault.address, strategy.address) == (False, 0)
...
def test_add_reward_refund(daddy, vault, strategy, refund_accountant):
    ...
    with ape.reverts("not added"):
        accountant.setRewardRefund(vault, strategy, True, amount, sender=daddy)
    ...
    tx = accountant.setRewardRefund(vault, strategy, True, amount, sender=daddy)

    event = list(tx.decode_logs(accountant.UpdateRewardRefund))
    ...
    assert accountant.rewardRefund(vault.address, strategy.address) == (True, 100)

    with ape.reverts("no refund and non zero amount"):
        accountant.setRewardRefund(vault, strategy, False, amount, sender=daddy)

    tx = accountant.setRewardRefund(vault, strategy, False, 0, sender=daddy)
...
def test_reward_refund__with_gain(
    ...
    accountant.updateDefaultConfig(0, 1_000, 0, 100, 10_000, 0, sender=daddy)
    assert accountant.rewardRefund(vault.address, strategy.address) == (False, 0)
    ...
    tx = accountant.setRewardRefund(vault, strategy, True, refund, sender=daddy)
    ...
    assert accountant.rewardRefund(vault.address, strategy.address) == (True, refund)
