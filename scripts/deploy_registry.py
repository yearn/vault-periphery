...
def deploy_release_and_factory():
    ...
    deployer_contract = project.Deployer.at("0x488E1A80133870CB71EE2b08f926CE329d56B084")
    ...
    release_deploy_bytecode = HexBytes(
        HexBytes(release_registry.contract_type.deployment_bytecode.bytecode) + release_constructor
        )
    
    print(f"Deploying Release Registry...")

    release_tx = deployer_contract.deploy(release_deploy_bytecode, salt, sender=deployer)
    ...
    factory_deploy_bytecode = HexBytes(
        HexBytes(factory.contract_type.deployment_bytecode.bytecode) + factory_constructor
        )
    
    factory_tx = deployer_contract.deploy(factory_deploy_bytecode, salt, sender=deployer)
